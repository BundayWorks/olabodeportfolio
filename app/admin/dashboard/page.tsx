  ...
  return (
    <div className="admin-page">
      ...
      <div style={{ marginBottom: '1.75rem' }}>
        <p style={{ fontSize: '0.8rem', color: 'var(--a-text3)', fontWeight: 500, marginBottom: '0.25rem', textTransform: 'uppercase', letterSpacing: '0.07em' }}>
          {new Date().toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' })}
        </p>
        <h1 style={{ fontSize: '1.6rem', fontWeight: 700, color: 'var(--a-text)', marginBottom: '0.25rem' }}>
      ...
      <div className="admin-stats">
        {[
          { label: 'Open', value: openCount ?? 0, color: 'var(--a-blue)', bg: 'var(--admin-blue-soft)' },
          { label: 'Done', value: doneCount ?? 0, color: 'var(--a-green)', bg: 'var(--admin-green-soft)' },
          { label: 'Achievements', value: achieveCount ?? 0, color: 'var(--a-amber)', bg: 'var(--admin-amber-soft)' },
        ].map(s => (
      ...
      {total > 0 && (
        <div className="admin-card admin-card--padded" style={{ marginBottom: '1.25rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--a-text)' }}>Completion rate</span>
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--a-green)' }}>{completionRate}%</span>
          </div>
          <div className="progress-bar">
            <div className="progress-bar__fill" style={{ width: `${completionRate}%`, background: 'var(--a-green)' }} />
          </div>
          <p style={{ fontSize: '0.75rem', color: 'var(--a-text3)', marginTop: '0.5rem' }}>
      ...
      <div className="overview-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
        ...
        <div className="admin-card" style={{ gridColumn: recentAchievements?.length === 0 ? '1/-1' : undefined }}>
          <div style={{ padding: '1rem 1.25rem 0.75rem', borderBottom: '1px solid var(--a-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '0.9rem', fontWeight: 700 }}>Open todos</span>
            <Link href="/admin/dashboard/todos" style={{ fontSize: '0.75rem', color: 'var(--a-blue)', textDecoration: 'none', fontWeight: 600 }}>
              View all →
            </Link>
          </div>
              ...
              (recentTodos ?? []).map((t: any) => (
                <div key={t.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.6rem', padding: '0.5rem 0', borderBottom: '1px solid #fafafa' }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: '0.85rem', fontWeight: 500, color: 'var(--a-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {t.title}
                    </div>
                    {t.commitments && (
                      <div style={{ fontSize: '0.72rem', color: 'var(--a-text3)' }}>{t.commitments.name}</div>
                    )}
                  </div>
                </div>
              ))
        ...
        <div className="admin-card">
          <div style={{ padding: '1rem 1.25rem 0.75rem', borderBottom: '1px solid var(--a-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Link href="/admin/dashboard/achievements" style={{ fontSize: '0.75rem', color: 'var(--a-blue)', textDecoration: 'none', fontWeight: 600 }}>
              View all →
            </Link>
          </div>
          <div style={{ padding: '0.75rem 1.25rem' }}>
              ...
              (recentAchievements ?? []).map((a: any) => (
                <div key={a.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.6rem', padding: '0.5rem 0', borderBottom: '1px solid #fafafa' }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '0.72rem', color: 'var(--a-text3)' }}>{a.date}</div>
                  </div>
                </div>
              ))
            )}
      ...
      <div style={{ marginTop: '1.25rem', display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
        {[
          ...
          <Link key={l.href} href={l.href} style={{
            ...
            fontSize: '0.82rem', color: 'var(--a-text)', textDecoration: 'none',
            border: '1.5px solid var(--a-border)', borderRadius: '8px',
            padding: '0.5rem 1rem', fontWeight: 600, background: 'var(--a-surface)',
